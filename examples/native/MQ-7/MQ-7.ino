 ...
 *
 * @file        MQ-7.ino
 * @brief       Example for reading gas measurements from MQ7 sensor (native)
 *
...
void setup()
{
   ...
   mq7.begin();
    ...
    bool calibrationResult=mq7.calibrateSensor(numOfCalibrations);
...
void loop()
{
  mq7.update();      // Update data, read voltage level from sensor
  Serial.println("CO: " + String(mq7.readSensor())+"ppm"); // Print the readings to the serial monitor
